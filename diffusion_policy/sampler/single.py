...
def coherence_sampler(policy, prior, obs_dict, num_sample=10, beta=0.5, dist_dir=None):
    """
    ...
    for key in obs_dict.keys():
        ...
        if key == 'prior':
            ...
            obs_dict_batch[key] = obs_dict[key].unsqueeze(1).repeat(1, num_sample, 1, 1).view(B * num_sample, PH, PD)
            # ipdb.set_trace()
        else:
    ...
    if 'action_obs_pred' in action_dict_batch and action_dict_batch['action_obs_pred'].numel() > 0:
        action_dict_batch['action_obs_pred'] = action_dict_batch['action_obs_pred'].view(B, num_sample, AH, OD)
    if 'obs_pred' in action_dict_batch and action_dict_batch['obs_pred'].numel() > 0:
        action_dict_batch['obs_pred'] = action_dict_batch['obs_pred'].view(B, num_sample, PH, OD)
    ...
    if dist_dir:
        output_dir = "result/distances"
        os.makedirs(output_dir, exist_ok=True)
        with open(os.path.join(output_dir, dist_dir), "a") as file:
            file.write(f"{dist.cpu().numpy()}\n")
