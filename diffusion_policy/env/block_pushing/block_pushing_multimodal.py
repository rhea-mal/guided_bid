...
class BlockPushMultimodal(block_pushing.BlockPush):
    ...
    def step(self, action):
        print('PERTURBATION by ', self.perturb)
        for i in range(len(self._block_ids)):
            current_translation, current_orientation = self._pybullet_client.getBasePositionAndOrientation(self._block_ids[i])
            perturbed_translation = list(current_translation)
            
            # Apply perturbation only to x and y
            perturbed_translation[0] += self.perturb # Perturb x
            perturbed_translation[1] += self.perturb # Perturb y

            # Set the perturbed position back
            self._pybullet_client.resetBasePositionAndOrientation(
                self._block_ids[i], 
