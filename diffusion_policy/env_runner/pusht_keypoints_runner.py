    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                # # noise
                # if self.noise > 0.0 and torch.rand(1) < 0.5 and past_action is not None:
                #     rand_steps = 10
                #     min_noise = self.noise * 0.5
                #     max_noise = self.noise * 1.5
                #     noise_range = max_noise - min_noise

                #     random_noise = np.random.rand(past_action.shape[0], 1, past_action.shape[2])
                #     scaled_noise = min_noise + noise_range * random_noise
                #     random_noise = scaled_noise * np.random.choice([-1, 1], size=(past_action.shape[0]))[:, None, None]
                #     random_noise = np.repeat(random_noise, rand_steps, axis=1)
                #     rand_action = past_action[:, -1:, :] + random_noise
                ...
                with torch.no_grad():
                    ...
                    if self.sampler == 'random':
                        ...
                        action_dict = contrastive_sampler(policy, self.weak, obs_dict, self.n_samples, self.sampler)
                ...
                # noise
                if self.noise > 0.0:
                    noise_seed = (np.random.rand(np_action_dict['action_pred'].shape[0], 1, np_action_dict['action_pred'].shape[2]) + 0.5) * np.random.choice([-1, 1], size=(np_action_dict['action_pred'].shape[0], 1, np_action_dict['action_pred'].shape[2]))
                    action_step = (np_action_dict['action_pred'][:, 1:] - np_action_dict['action_pred'][:, :-1])
                    noise_step = noise_seed.repeat(action_step.shape[1], axis=1) * action_step
                    noise_cum = np.cumsum(noise_step, axis=1)
                    action += noise_cum[:, :action.shape[1]] * self.noise
