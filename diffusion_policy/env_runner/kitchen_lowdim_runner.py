    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                # # noise
                # if self.noise > 0.0 and torch.rand(1) > 0.5:
                #     scale = policy.normalizer['action'].get_input_stats()['std'].mean().item()
                #     # action_noise = exponential_noise(action_dict['action_pred'].shape, self.noise) * scale
                #     action_noise = constant_noise(action_dict['action_pred'].shape, self.noise) * scale
                #     action += action_noise[:, :action.shape[1]]
